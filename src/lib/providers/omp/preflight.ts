...
export const runOmpPreflight = async (): Promise<IAgentPreflight> => {
  ...
  try {
    const { stdout } = await execFile('omp', ['--version'], { timeout: CMD_TIMEOUT, env });
    installed = true;
  ...
  try {
    const { stdout } = await execFile('which', ['omp'], { timeout: CMD_TIMEOUT, env });
