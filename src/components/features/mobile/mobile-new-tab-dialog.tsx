...
const MobileNewTabDialog = ({ open, onOpenChange, onCreateTab }: IMobileNewTabDialogProps) => {
  ...
  const MENU_ITEMS = [
    ...
    { key: 'pi', type: 'pi-cli' as const, icon: <PiIcon className="h-5 w-5" />, label: 'Pi', startCommand: 'pi-new' as const },
  ...
  const handleStartNew = async (item: Extract<(typeof MENU_ITEMS)[number], { startCommand: 'claude-new' | 'codex-new' | 'pi-new' }>) => {
